-- ╔══════════════════════════════════════════╗
-- ║     Sols RNG Egg Farm  |  v4.0           ║
-- ║     Chat Detection + Tabbed UI           ║
-- ╚══════════════════════════════════════════╝

do
    local old = game:GetService("CoreGui"):FindFirstChild("SolsEggFarmGUI")
...
-- Slider helper (returns value label for external read)
function UI.slider(parent, yPos, labelTxt, minV, maxV, settingKey)
    ...
    local valD = UI.label(f, {
        Size=UDim2.new(0,40,1,0), Position=UDim2.new(1,-48,0,0),
        Text=tostring(SETTINGS[settingKey]), TextColor3=C.accent, TextSize=12
    })
    local track = Instance.new("Frame", f)
    track.Size=UDim2.new(0.3,0,0,4); track.Position=UDim2.new(0.52,0,0.5,-2)
    track.BackgroundColor3=C.bg; track.BorderSizePixel=0; UI.corner(track,4)
    local fill = Instance.new("Frame", track)
    fill.BackgroundColor3=C.accent; fill.BorderSizePixel=0; UI.corner(fill,4)
    local knob = Instance.new("Frame", track)
    knob.Size=UDim2.new(0,12,0,12)
    knob.BackgroundColor3=Color3.new(1,1,1); knob.BorderSizePixel=0; UI.corner(knob,6)
    ...
    knob.InputBegan:Connect(startSlide)
...
local sprintLbl = UI.label(sprintCard, {
...
local sprintIndicator = UI.label(sprintCard, {
    ...
    Text="OFF", TextColor3=C.red, TextSize=12
})
...
-- Sprint logic (hold Shift)
local BASE_SPEED = 16
UserInputService.InputBegan:Connect(function(inp, gp)
    if gp then return end
    if inp.KeyCode == Enum.KeyCode.LeftShift and SETTINGS.SPRINT_SPEED > 0 then
        STATE.sprintActive = true
        if humanoid and humanoid.Parent then
            humanoid.WalkSpeed = BASE_SPEED + SETTINGS.SPRINT_SPEED
        end
        sprintIndicator.Text = "ON"
        sprintIndicator.TextColor3 = C.green
    end
end)
UserInputService.InputEnded:Connect(function(inp)
    if inp.KeyCode == Enum.KeyCode.LeftShift then
        STATE.sprintActive = false
        if humanoid and humanoid.Parent and not STATE.running then
            humanoid.WalkSpeed = BASE_SPEED
        end
        sprintIndicator.Text = "OFF"
        sprintIndicator.TextColor3 = C.red
    end
end)
